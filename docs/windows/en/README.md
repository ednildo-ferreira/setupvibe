...
The Windows Edition configures a complete native Windows development environment with WinGet as the primary package source and Chocolatey for packages not available through WinGet.
...
7. Restart Windows if the summary requests it.
...
3. Configures persistent user `PATH` entries.
4. Installs OpenSSH Client, WinGet, and Chocolatey when needed.
5. Installs each Windows package independently and continues after isolated package failures.
6. Installs the PHP, Ruby, Node.js, Python, and Rust ecosystem tools.
7. Configures Starship and zoxide for Windows PowerShell and PowerShell 7.
8. Displays a final summary and the transcript log location.
...
1. Restart Windows when requested.
